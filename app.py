...
with st.expander("➕ Add New Customer"):
    ...
    if st.button("Add Customer"):
        if not a_name or not a_phone:
            ...
            res = requests.post(f"{API_BASE}/add", json={"name": a_name, "phone": a_phone, "paid": a_paid.lower()})
...
with f_col:
    st.session_state.filter_status = st.selectbox("Filter", ["All", "Paid", "Unpaid"],
...
def date_or_none(iso): return date.fromisoformat(iso) if iso else None
...
for idx, row in enumerate(rows):
    ...
    if editing:
        ...
        st.success(f"✅ Paid: {last_paid} | Next: {next_pay}") if status=='paid' else st.error('❌ Not Paid')
        c1,c2,c3=st.columns(3)
        if c1.button('✏️ Edit', key=f'e{idx}'): st.session_state.edit_idx=idx; st.rerun()
        if c2.button('Toggle Paid', key=f't{idx}'): requests.post(f"{API_BASE}/toggle_paid", json={'name': name}); st.rerun()
        if c3.button('🗑️ Del', key=f'd{idx}'): requests.post(f"{API_BASE}/delete", json={'name': name}); st.rerun()
